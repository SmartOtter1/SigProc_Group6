...
function [rfModel, rfResults, knnModel, knnResults] = trainModel_LOO_wrapped(allFeatures, allLabels, params, fsIndex)

    % Initialize new cell array to store reduced features
    selectedFeatures = cell(size(allFeatures));

    % Loop through each cell
    for i = 1:numel(allFeatures)
        selectedFeatures{i} = allFeatures{i}(:, fsIndex); % Apply selection
    end
    
    % initialize model creation, RF and KNN model created
    [rfModel, rfResults] = trainRFModel_LOO(selectedFeatures, allLabels, params);
    [knnModel, knnResults] = trainKNNModel_LOO(selectedFeatures, allLabels, params);
end
